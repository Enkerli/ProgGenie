#!/usr/bin/env python3
"""Checks ProgGenie's audio component identity.
...
MelGen's project file, so it started life claiming MelGen's triple, and the
codes are forever. It is `aumi/PgGn/Enke` — deliberately *not* `Prst`, which
belongs to the JUCE Progression Studio and would collide with the AUv3 that
build already ships. The two are meant to be installable side by side.
...
Checks:
  ...
  4. The display name follows the "Manufacturer: Product" convention hosts
     expect, so AUM shows "ProgGenie" rather than a target name.
...
INFO_PLIST = REPO / "ProgGenieExtension" / "Info.plist"
PROJECT = REPO / "ProgGenie.xcodeproj" / "project.pbxproj"
HOST_MODEL = REPO / "ProgGenie" / "Model" / "AudioUnitHostModel.swift"
